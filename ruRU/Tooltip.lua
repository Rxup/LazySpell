...
----
        "When applied to your fishing pole, increases Fishing by (.*) for (.*) minutes."
...
----    
        ...
        "Обучает пошиву %1.",
----
        "Если насадить на крючок, ваш навык рыбной ловли будет увеличен на %1 на %2 мин."
        ...
        if st ~= nil then
            for i, stat  in ipairs(stats) do
                if string.find(st, stat) then
                    ...
                    if (string.len(stats[i]) - string.len(statsRU[i])) > 11 then
                        GameTooltip:SetHeight(GameTooltip:GetHeight() - (GameTooltip:GetHeight() - 36) / GameTooltip:NumLines());
                    elseif (string.len(statsRU[i]) - string.len(stats[i])) > 11 then
                        GameTooltip:SetHeight(GameTooltip:GetHeight() + (GameTooltip:GetHeight() - 36) / GameTooltip:NumLines());
                    end
